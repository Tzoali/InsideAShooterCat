#if !defined (_GAME_RUN2_H_)
#include "menu.h"

int touch2(SDL_Rect a,SDL_Rect b,SDL_Rect ta,SDL_Rect tb)
{
...
SDL_Texture *LoadTexture2(string filePath, SDL_Renderer *renderTarget)
{
...
void new_bala2(balas *b,SDL_Rect position,int *noBalas,bool *shootUP,int direccion,int bandera,SDL_Rect rectBullet2,SDL_Texture *bullet2,SDL_Texture *bullet1,SDL_Rect rectBullet1)
{
...
void evilBehavior2(SDL_Rect *evilRect,SDL_Rect *evilPosition, SDL_Rect goodPosition,SDL_Rect goodRect)
{
    int direccion=0;
    EvilStatus dir;
    ...
    if(dir==JUMP2)
        (*evilPosition).x+=((*evilPosition).x<goodPosition.x ? 3+rand()%6 : -3-rand()%6);
}
void imprimirVillano2(enemigo *e,SDL_Rect *playerPosition,SDL_Rect rectImage,int z,bool *isRunning,SDL_Renderer *renderTarget,game_state *State,int contshot, game_assets *Assets)
{
    for(int i=0;i<NUM_ENEMIGOS;i++)
    {
        if(e[i].die==0)
        {
            ...
            evilBehavior2(&e[i].rectVillain,&e[i].villainPosition,*playerPosition,rectImage);
...
void movimientosProtagonista2(SDL_Rect *playerPosition,int velx,int vely,bool *salto,float *frameTime,float deltaTime,int *z,int *contshot,enemigo *e,bool active,Status *dir,bool *draw,SDL_Rect *rectImage,int direccion,int *shot)
{
...
static void GameRun2(SDL_Renderer *Render, game_state *State, game_assets *Assets, game_input *Input,bool restart)
{
    SDL_Rect qwerty;
    if(restart)
    {
        ...
        Picos[1].trampaPosition.x=1418;
        State->Player.X=30;
        State->Player.Y=0;
        Assets->Map.Quads.x=0;
        Assets->Map.Tex=Assets->Map3.Tex;
        Assets->Clouds.Quads.x=0;
        Assets->Clouds.Tex=Assets->Clouds2.Tex;
        for(int i=0;i<NUM_ENEMIGOS;i++)
        {
            e[i].villain=Assets->Monster2.Tex;
            ...
            e[i].rectVillain.w=Assets->Monster.Quads.w =32;
            e[i].rectVillain.h=Assets->Monster.Quads.h=42;
    ...
    if(Controller->MoveRight.Down)
    {
          ...
          else
          {
             if(Assets->Map.Quads.x<1412)
            {
                if(State->premios[1]==1)
                   ...
                   for(int i=0;i<2;i++)
                        Picos[i].trampaPosition.x-=(Img ? 11 : 22);
                ...
                if(shootUp && contshot >= 7)
                {
                    ...
                    if(dir != DOWN)
                    {
                        if(Img)
                        {
                            ...
                            new_bala2(b+noBalas, player, &noBalas, &shootUp, direccion, 1, Assets->BulletOne.Quads, Assets->BulletOne.Tex, Assets->BulletOne.Tex, Assets->BulletOne.Quads);
                        }
                        else
                        {
                        ...
                        if(!Img && State->kame>=50 && velY==0)
                            {
                                ...
                                new_bala2(b+noBalas, player, &noBalas, &shootUp, direccion, 0, Assets->BulletOne.Quads, Assets->BulletOne.Tex, Assets->BulletOne.Tex, Assets->BulletOne.Quads);
                                State->kame-=50;
                            }
                    }
                }
        ...
        movimientosProtagonista2(&player,velX,velY,&salto,&frametime,State->dtInput,&z,&contshot,e,active,&dir,&draw,&Current.Quads,direccion,&shoot);
        ...
        imprimirVillano2(e,&player,Current.Quads,z,&Running,Render,State,contshot,Assets);
        State->Player.Y=player.y;
        State->Player.X=player.x;
        ...
        for(int i=0;i<2;i++)
        {
            SDL_RenderCopy(Render,Picos[i].trampaTex,NULL,&Picos[i].trampaPosition);
            qwerty.x=State->Player.X;
            qwerty.y=State->Player.Y;
            qwerty.w=95;
            qwerty.h=25;
            if(touch(qwerty,Picos[i].trampaPosition,player,qwerty) && !(Img && dir==DOWN))
            {
                State->Life++;
                Mix_PlayChannel(7,Assets->Espinazo,0);
                if(State->faltas[1]!='9')
                    State->faltas[1]++;
                else
                {
        ...
        if(Assets->Map.Quads.x>=1419 && player.x==550 && State->kills==2*NUM_ENEMIGOS)
        {
            State->CurrentState=GAME_PLAY3;
        }
}

#define _GAME_RUN2_H_
#endif
